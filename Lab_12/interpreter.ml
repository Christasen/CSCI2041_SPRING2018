...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | For (string1, lower, upper, s1) ->
      (match eval lower stt, eval upper stt with
      | Int v1, Int v2 -> if v1 > v2 then stt
                          else exec (For(string1, Val(Int(v1+1)),
                          Val(Int(v2)), s1))
                          (exec s1 (exec (Assign(string1, lower)) stt))
      | _, _ -> raise (Failure "Incompatible types on For"))
