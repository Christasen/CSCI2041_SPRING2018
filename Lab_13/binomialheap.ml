...
module Int : (OrderedSig with type t = int) = struct
    type t = int
    let eq t1 t2 : bool = t1 = t2
    let lt t1 t2 : bool = t1 < t2
    let leq t1 t2 : bool = t1 <= t2
  end
...
module BinomialHeap (Od: OrderedSig):
    ...
    let deleteMin t = let (Node (_, x, t1), t2) = removeMinTree t
				   in (merge (List.rev t1) t2)

    let findMin t: elem = let (xs, _) = removeMinTree t in root xs
